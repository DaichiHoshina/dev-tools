...
# kube-deploy — dev環境イメージオーバーライドCLI
#
# Usage:
...
cmd_diff() {
  ...
  for svc in $(list_services); do
    ...
    local current_tag
    current_tag="$(get_current_image_tag "$svc" "$ns" 2>/dev/null)" || current_tag="unknown"

    if [[ "$current_tag" != "latest" && "$current_tag" != "unknown" ]]; then
      if [[ "$has_diff" == false ]]; then
        print_header "オーバーライド中のサービス"
        has_diff=true
        diff_rows="サービス\tタグ\tチケット\tデプロイ者\tMR\n"
      fi
      ...
      # ConfigMapに記録がない場合、タグのコミットSHAからGitLab APIで補完
      if [[ "$deployed_by" == "-" ]]; then
        local sha=""
        if [[ "$current_tag" =~ ^dev-([0-9a-f]{7,40})$ ]]; then
          sha="${BASH_REMATCH[1]}"
        fi
        if [[ -n "$sha" ]]; then
          local project_path="${SERVICE_GITLAB_PROJECT[$svc]:-}"
          if [[ -n "$project_path" ]]; then
            local encoded_path="${project_path//\//%2F}"
            local commit_author
            commit_author="$(glab api "/projects/${encoded_path}/repository/commits/${sha}" 2>/dev/null | jq -r '.author_name // empty' 2>/dev/null)" || true
            if [[ -n "$commit_author" ]]; then
              deployed_by="$commit_author"
            fi
          fi
        fi
      fi
      ...
      diff_rows+="${ns}/${svc}\t${short_tag}\t${ticket}\t${deployed_by}\t${mr_url}\n"
    fi
  done
...
reset_single_service() {
  ...
  if [[ "$original_tag" == "null" || -z "$original_tag" ]]; then
    log_warn "${svc}: original_tag が不明です。ConfigMapエントリのみ削除します"
    remove_configmap_override "$svc"
    log_success "${svc}: ConfigMapエントリを削除しました"
    return 0
  fi
...
cmd_set() {
  ...
  # 現在のタグを取得
  local current_tag
  current_tag="$(get_current_image_tag "$service" "$ns")" || current_tag="unknown"
...
cmd_deploy_ticket() {
  ...
  for entry in "${deploy_plan[@]}"; do
    ...
    # 現在のタグを保存
    local current_tag
    current_tag="$(get_current_image_tag "$svc" "$ns")" || current_tag="unknown"
...
check_conflict() {
  ...
  if [[ -n "$entry" ]]; then
    # 実際のDeploymentタグを確認し、latestならConfigMapエントリは古いのでクリーンアップ
    local ns="${SERVICE_NAMESPACE[$service]}"
    local current_tag
    current_tag="$(get_current_image_tag "$service" "$ns" 2>/dev/null)" || current_tag="unknown"

    if [[ "$current_tag" == "latest" ]]; then
      log_info "${service}: 実際のデプロイはlatestのため、古いConfigMapエントリを削除します"
      remove_configmap_override "$service"
      return 0
    fi
